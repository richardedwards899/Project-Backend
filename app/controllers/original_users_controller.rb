class OriginalUsersController < ApplicationController
  ...
  def index
    users = OriginalUser.all
    render :json => users
  end

  def show
    user = OriginalUser.find(params[:id])
    render :json => user
  end
  ...
  def show_user_reports
    reports = OriginalUser.find(params[:id]).reports
    render :json => reports
  end

  # Show a particular report for one user. Corresponds to: GET 'users/:id/reports/:r_id' => 'users#show_report'
  def show_report
    report = OriginalUser.find(params[:id]).reports.find(params[:r_id])
    render :json => report
  end
  ...
  def show_report_inputs
    inputs = OriginalUser.find(params[:id]).reports.find(params[:r_id]).inputs
    render :json => inputs
  end
...
# Show one input for a particular report. Corresponds to: GET 'users/:id/reports/:r_id/inputs/:i_id' => 'users#show_report_input'
  def show_report_input
    input = OriginalUser.find(params[:id]).reports.find(params[:r_id]).inputs.find(params[:i_id])
    render :json => input
  end
